...
export async function analyzeProduct(
  ...
  let salesEstimationFailed = false;
  const [productRaw, offersRaw, salesRaw] = await Promise.all([
    ...
    fetchWithCache(
      ...
      CACHE_TTL.BSR,
    ).catch((): RainforestSalesEstimationResponse => {
      salesEstimationFailed = true;
      return {
  ...
  return {
    ...
    alerts,
    salesEstimate,
    salesEstimateUnavailable: salesEstimationFailed || undefined,
    bsr,
    analyzedAt: Date.now(),
  };
}
